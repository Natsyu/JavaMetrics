 ...
 * If the same attribute is used in multiple methods, it is counted multiple times (one per method even if used several times inside one).
 */
    ...
    public List<Metric<Integer>> compute(CompilationUnit compilationUnit) {
        ...
        ClassMetric<Integer> atfd = c -> {
            ...
            c.findAll(FieldAccessExpr.class).stream()
                    ...
                    .forEach(fe -> foreignAttributeUsages.add(
                            fe.resolve().asField().declaringType().getQualifiedName() + "#" + fe.getNameAsString()
                    ));
