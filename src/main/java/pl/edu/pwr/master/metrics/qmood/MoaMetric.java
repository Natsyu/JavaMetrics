...
public class MoaMetric extends ClassMetricStrategy<Integer> {
    ...
    public List<Metric<Integer>> compute(CompilationUnit compilationUnit) {
        ClassMetric<Integer> moa = c -> {
            List<FieldDeclaration> fields = c.getFields();
            return (int) fields.stream().filter(f -> {
                try {
                    return isUserDefined(f.getElementType().toString(), f.resolve().getType().describe());
    ...
    private boolean isUserDefined(String fieldTypeName, String fieldDescribe) throws IOException, ParseException, JavaParserNotConfiguredException {
