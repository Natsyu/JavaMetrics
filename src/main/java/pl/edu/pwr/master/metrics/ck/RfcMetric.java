...
public class RfcMetric extends ClassMetricStrategy<Integer> {
    ...
    public List<Metric<Integer>> compute(CompilationUnit compilationUnit) {
        ClassMetric<Integer> rfc = c -> {
            List<String> methodNames = c.findAll(MethodDeclaration.class).stream()
                    .map(NodeWithSimpleName::getNameAsString)
                    .collect(Collectors.toList());

            List<String> methodsCalled = c.findAll(MethodCallExpr.class).stream()
