...
public class CsvProcessor {
    ...
    private static final String[] headerValuesRolledUp = {"Project", "Package", "Class", "MethodSignature", "OuterClass", "AccessModifier", "IsStatic", "IsFinal"};
    private static final String DEFAULT_CSV_NAME = "smells.csv";

    private CsvProcessor() {
    }

    public static void write(String fileLocation, String projectName, Map<MetricDetail, TreeMap<String, Metric<?>>> rolledUpMetrics, List<String> metricNames) throws IOException {
        ...
        if (f.exists() && f.canRead()) {
            ...
            try (FileWriter fileWriter = new FileWriter(f)) {
                String header = String.join(",", headerValuesRolledUp) + "," +
    ...
    private static void writeRow(String projectName, FileWriter fileWriter, Map<MetricDetail, TreeMap<String, Metric<?>>> rolledUpMetrics) throws IOException {
        rolledUpMetrics.keySet().forEach(k -> {
            List<String> details =
            ...
            rolledUpMetrics.get(k).forEach((ki, v) -> {
                if (v == null)
                    stringJoiner.add("");
                else
                    stringJoiner.add(v.getValue().toString());
            });
            try {
