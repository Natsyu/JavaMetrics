...
public abstract class MethodMetricStrategy<T> implements MetricStrategy<T> {
     ...
     */
    protected List<Metric<T>> getMetricForMethod(MethodMetric<T> mm, CompilationUnit compilationUnit) {
     ...
     */
    protected List<Metric<T>> getMetricForConstructor(ConstructorMetric<T> cc, CompilationUnit compilationUnit) {
     ...
     */
    protected List<Metric<T>> getMetricForCallable(MethodMetric<T> mm, ConstructorMetric<T> cc, CompilationUnit compilationUnit) {
        List<Metric<T>> output = deriveMethodMetrics(mm, compilationUnit);
        output.addAll(deriveConstructorMetrics(cc, compilationUnit));
        return output;
    }

    private List<Metric<T>> deriveConstructorMetrics(ConstructorMetric<T> cc, CompilationUnit compilationUnit) {
    ...
    private Optional<String> getOuterClassName(CallableDeclaration m) {
        return getParentClass(m)
                .flatMap(this::getParentClass)
                .map(NodeWithSimpleName::getNameAsString);
    }
}
